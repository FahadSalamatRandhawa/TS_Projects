...
let feeStructure = {
    'Block Chain': -200, 'AIC': -250, 'CNC': -140, 'IOT': -590, 'MW3': -210, '5G': -160, 'Computational Genomics': -300
};
let studentsList = [];
let mainQuestion = {
    name: 'mainSelection',
...
let courseQuestion = {
    ...
    choices: ['Block Chain', 'AIC', 'CNC', 'IOT', 'MW3', '5G', 'Computational Genomics']
};
let customInput = {
    name: 'input',
    type: 'input'
        ...
        do {
            ...
            switch (mainChoice) {
                case 'New Student': {
                    ...
                    let courseChoice = (yield inquirer.prompt(courseQuestion)).courseSelection;
                    let tempStudent = {
                        id: newID,
                        name: newName,
                ...
                case 'de enroll': {
                    customInput.message = "Student ID : ";
                    let studntID = (yield inquirer.prompt(customInput)).input;
                    let de_Enroll = (yield inquirer.prompt(courseQuestion)).courseSelection;
                    studentsList.forEach(student => {
                        if (student.id === studntID) {
